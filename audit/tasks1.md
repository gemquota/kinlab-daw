# KinLab DAW — Development Plan & Task List

**Generated from:** review1.md  
**Date:** 2026-07-26  
**Total Tasks:** 47  
...
| ID | Task | File | Status |
|----|------|------|--------|
| 1.2.1 | Wrap `GlassSliderMini` with `React.memo` | `src/pages/Waveform.tsx` | ⬜ |
| 1.2.2 | Wrap `SettingsPanel` with `React.memo` | `src/pages/Waveform.tsx` | ⬜ |

---

## Phase 2: Component Extraction (P2)

### 2.1 — Extract SettingsPanel
| ID | Task | File | Status |
|----|------|------|--------|
| 2.1.1 | Create `src/components/daw/SettingsPanel.tsx` with extracted component | `src/components/daw/SettingsPanel.tsx` | ⬜ |
| 2.1.2 | Move `GlassSliderMini` to `src/components/common/GlassSlider.tsx` | `src/components/common/GlassSlider.tsx` | ⬜ |
...
| ID | Task | File | Status |
|----|------|------|--------|
| 3.2.1 | Add `onError` handler to Zustand persist middleware in `daw.store.ts` | `src/store/daw.store.ts` | ⬜ |
| 3.2.2 | Add `onError` handler to Zustand persist middleware in `settings.store.ts` | `src/store/settings.store.ts` | ⬜ |
| 3.2.3 | Add `onError` handler to Zustand persist middleware in `session.store.ts` | `src/store/session.store.ts` | ⬜ |
...
| ID | Task | File | Status |
|----|------|------|--------|
| 4.3.1 | Add `role="dialog"` to side panel overlay | `src/pages/Waveform.tsx` | ⬜ |
| 4.3.2 | Add `aria-modal="true"` to side panel | `src/pages/Waveform.tsx` | ⬜ |
| 4.3.3 | Add `aria-labelledby` for panel title | `src/pages/Waveform.tsx` | ⬜ |
...
## Phase 6: Code Quality (P6)

### 6.1 — Console Statement Cleanup
| ID | Task | File | Status |
|----|------|------|--------|
| 6.1.1 | Find and remove/wrap console.log in audioEngine.ts | `src/audio/audioEngine.ts` | ⬜ |
| 6.1.2 | Find and remove/wrap console statements in other files | `src/**/*.{ts,tsx}` | ⬜ |

### 6.2 — TypeScript Strictness
| ID | Task | File | Status |
|----|------|------|--------|
| 6.2.1 | Replace `any` types with proper types in audio engine callbacks | `src/audio/audioEngine.ts` | ⬜ |
| 6.2.2 | Fix `trackCounter` module-level variable — use store or ref | `src/store/daw.store.ts` | ⬜ |

---

## Phase 7: Documentation (P7)
...
|-------|-------|----------|
| Phase 1: Performance | 7 | 🔴 Critical |
| Phase 2: Component Extraction | 4 | 🟡 High |
| Phase 3: Error Handling | 8 | 🟡 High |
| Phase 4: Accessibility | 9 | 🟡 Medium |
| Phase 5: Security | 2 | 🟢 Low |
| Phase 6: Code Quality | 4 | 🟢 Low |
| Phase 7: Documentation | 9 | 🟢 Low |
| Phase 8: Testing | 9 | 🟡 High |
| **Total** | **52** | |

---

*Tasks generated from review1.md audit findings.*
