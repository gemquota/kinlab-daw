import { getAudioContext, getDrumGain } from "@/audio/audioEngine";

/**
 * Filthy techno drum synthesis engine.
 * Each drum type routes through its own gain node → preGain → compressor → effects → output.
 */
...
export function triggerKick(when?: number): void {
  ...
  osc.connect(dist); dist.connect(lp); lp.connect(body); body.connect(out);
...
export function triggerHat(open = false, when?: number): void {
  ...
  src.connect(bp); bp.connect(hp); hp.connect(env);
  src.connect(res); res.connect(resG); resG.connect(env);
  env.connect(sat); sat.connect(out);
...
export function triggerClap(when?: number): void {
  ...
  osc.connect(tEnv); tEnv.connect(out);
...
export function triggerBass(freq = 55, when?: number): void {
  ...
  osc.connect(filter); filter.connect(sat); sat.connect(amp); amp.connect(out);
